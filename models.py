...
def collideSnake(head, pellet):
    if head == None:
        return False
    if collided(head, pellet):
        return True
    return collideSnake(head.tail, pellet)
...
class Pellet:
    def __init__(self, width, height):
        self.width = width
        self.height = height
        self.x = random.randint(0, width-(BLOCKSIZE+10))
        self.y = random.randint(0, height-(BLOCKSIZE+10))
    ...
    def randomizePos(self):
        self.x = random.randint(0, self.width-(BLOCKSIZE+10))
        self.y = random.randint(0, self.height-(BLOCKSIZE+10))
